...
const CONTROLLER_ERROR_MESSAGE =
	'Controllers must follow expected format. (See Notion)';
const CONTROLLER_EXPORTS_ERROR_MESSAGE =
	'Controllers must export a single async named function. (See Notion)';
...
module.exports = {
	...
	create(context) {
		...
		const expectedFunctionName = filename
					...
					context.report({
						...
						fix(fixer) {
							const sourceCode = context
								.getSourceCode()
								.getText(exportNode);
							const params = sourceCode.slice(
								sourceCode.indexOf('(', 0) + 1,
								sourceCode.indexOf(')', 0)
							);
							let remaining;

							if (exportNode.type === 'ArrowFunctionExpression') {
								remaining = sourceCode.slice(
									sourceCode.indexOf('=>', 0) + 2
								);
							} else {
								remaining = sourceCode.slice(
									sourceCode.indexOf(params) +
										params.length +
										1
								);
							}
							...
							return fixer.replaceText(
								exportNode,
								`async function ${expectedFunctionName}(${params})${remaining}`
							);
						},
					});
				}
